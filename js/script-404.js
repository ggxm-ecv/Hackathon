(function () {
    ...
    let gameProcess = setTimeout(function () {
      ...
      var checkDead = setInterval(function () {
        ...
        obstacles.forEach((elem) => {
          ...
          if (
            obstacleLeft < leftTargetMax &&
            obstacleLeft > leftTargetMin &&
      ...
      obstacles.forEach((elem) => {
        let obstaclePosistion = parseInt(
          window.getComputedStyle(elem).getPropertyValue("left")
        );

        setInterval(() => {
          let obstacleNewPosistion = parseInt(
            window.getComputedStyle(elem).getPropertyValue("left")
          );
  ...
  function allDances() {
    ...
    rythm.addRythm("radius2", "radius", 0, 10, {
      reverse: true,
      min: 0,
      max: 30,
    });
  ...
  function checkKey(e) {
    ...
    if (e.keyCode == "38") {
      ...
      console.log("en bas");
      if (character.classList == "animateDown") {
        return;
      }
